...
/**
 * author: jiangwei18 on 17/3/30 20:12
 */
...
public class EventHandler extends Handler {
        ...
        while (clazz2 != EventHandler.class) {
            ...
            for (Class event : judgeInterfaces) {
                if (!event.getSimpleName().startsWith("EventOn")) {
                    throw new IllegalStateException("eventinterface 's name needs starts with EventOn");
                }
                messages.add(event);
            }
    ...
    public void handleMessage(Message msg) {
        ...
        if (method != null) {
            ...
            if (result != null && result instanceof Boolean && (msg.arg1 == SendType.TYPE_ONE) || msg.arg1 == SendType.TYPE_TAIL_ONE
                    || msg.arg1 == SendType.TYPE_BUT_ONE || msg.arg1 == SendType.TYPE_TAIL_BUT_ONE) {
                boolean isInterrupt = (boolean) result;
                // FALSE 不拦截
                Object[] a = (Object[]) msg.obj;
                if (!isInterrupt && (msg.arg1 == SendType.TYPE_ONE || msg.arg1 == SendType.TYPE_BUT_ONE)) {
                    // 按照正序下发
                    EventCenter.getInstance().send(eventCache.get(msg.what), getHandlerId(), SendType.TYPE_BUT_ONE, a);
                } else if (msg.arg1 == SendType.TYPE_TAIL_ONE || msg.arg1 == SendType.TYPE_TAIL_BUT_ONE) {
                    // 按照倒序下发
                    EventCenter.getInstance().send(eventCache.get(msg.what), getHandlerId(), SendType.TYPE_TAIL_BUT_ONE, a);
                }
            }
        } else {
