...
/**
 * author: jiangwei18 on 17/3/30 20:03
 */
...
public class EventCenter {
    ...
    public void send(@NonNull Class<? extends Event> event, int targetHandlerId, int sendType, Object...objs) {
        ...
        } else if (sendType == SendType.TYPE_ALL) {
            for (EventHandler handler : eventEntity.classHandlers) {
                android.os.Message message = handler.obtainMessage();
                message.what = eventEntity.classId;
                message.arg1 = SendType.TYPE_ALL;
                // obj 可以为空
                message.obj = objs;
                handler.sendMessage(message);
            }
        } else if (sendType == SendType.TYPE_TAIL_ALL) {
        ...
        } else if (sendType == SendType.TYPE_TAIL_ONE) {
            for (int index = size - 1; index >= 0; index--) {
                EventHandler handler = eventEntity.classHandlers.get(index);
                if (handler.getHandlerId() == targetHandlerId) {
                    android.os.Message message = handler.obtainMessage();
        ...
        } else if (sendType == SendType.TYPE_BUT_ONE) {
            for (EventHandler handler : eventEntity.classHandlers) {
                if (handler.getHandlerId() > targetHandlerId) {
                    android.os.Message message = handler.obtainMessage();
                    message.what = eventEntity.classId;
                    message.arg1 = SendType.TYPE_BUT_ONE;
                    // obj 可以为空
                    message.obj = objs;
                    handler.sendMessage(message);
                    break;
        ...
        } else if (sendType == SendType.TYPE_TAIL_BUT_ONE) {
            for (EventHandler handler : eventEntity.classHandlers) {
                if (handler.getHandlerId() < targetHandlerId) {
                    android.os.Message message = handler.obtainMessage();
                    message.what = eventEntity.classId;
                    message.arg1 = SendType.TYPE_TAIL_BUT_ONE;
                    // obj 可以为空
                    message.obj = objs;
                    handler.sendMessage(message);
                    break;
